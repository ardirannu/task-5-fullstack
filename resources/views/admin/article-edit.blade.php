...
<div class="row">
    <div class="col-12 grid-margin stretch-card">
        <div class="card">
          <div class="card-body">
            <h4 class="card-title">Form Edit Data Article</h4>
            <p class="card-description">
                Complete the form below to edit article data!
            </p>
            <form class="forms-sample" action="{{ route('article.update', $article->id)}}" method="POST" enctype="multipart/form-data">
                @csrf
                @method('patch')
                <div class="row">
                    <div class="col-6">
                        ...
                        <input type="text" name="title" class="form-control" id="exampleInputName1" placeholder="Title" value="{{ $article->title }}">
                        </div>
                    </div>
                    <div class="col-6">
                        <div class="form-group">
                        <label for="exampleInputName1">Content * @error('content') <b class="text-danger"> {{ "(".$message.")" }} </b> @enderror</label>
                        <input type="text" name="content" class="form-control" id="exampleInputName1" placeholder="Content" value="{{ $article->content }}">
                        </div>
                    </div>
                </div>
                <div class="row">
                        ...
                        <div class="form-group">
                            ...
                            <select name="user_id" class="form-control" id="exampleSelectGender">
                                ...
                                @foreach ($user as $data)
                                    <option value="{{ $data->id }}" @if($article->user_id == $data->id) selected @endif>{{ $data->name }}</option>    
                                @endforeach
                            </select>
                        </div>
                    </div>
                        ...
                        <div class="form-group">
                            ...
                            <select name="category_id" class="form-control" id="exampleSelectGender">
                                ...
                                @foreach ($category as $data)
                                    <option value="{{ $data->id }}" @if($article->category_id == $data->id) selected @endif>{{ $data->name }}</option>    
                                @endforeach
                            </select>
                        </div>
                    </div>
